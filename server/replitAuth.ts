...
export const requiresSubscription = (requiredTier: string): RequestHandler => {
  return async (req, res, next) => {
    ...
    const userLevel = tierLevels[dbUser.subscriptionTier as keyof typeof tierLevels] || 0;
    const requiredLevel = tierLevels[requiredTier as keyof typeof tierLevels] || 0;

    // Check trial period for free users
    if (dbUser.subscriptionTier === "free" && dbUser.trialEndDate && new Date() > dbUser.trialEndDate) {
